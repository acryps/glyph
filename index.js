...
webfontsGenerator({
	...
	writer.write(`import { Font, fontFamily, fontWeight, fontStyle, descendant, empty, before, content, attribute } from '@acryps/style';\n`);
	writer.write('\n');
	...
	writer.write(`export const icons = () => descendant('ui-icon') (\n`);
	writer.write(`\tfontFamily(iconFont.name),\n`);
	writer.write(`\tfontWeight('normal'),\n\n`);
	writer.write(`\tempty() ( before() (content('?'))),\n\n`);

	for (let icon of iconNames) {
		writer.write(`\tattribute('ui-${icon}') ( before() (content('\\f1${(iconNames.indexOf(icon) + 1).toString(16).padStart(2, '0')}')) ),\n`);
	}
